...
def results_list(request):
    """
    Show actual cycling race results only.

    - Filters by selected race
    - Shows only athletes marked as participated
    - Shows only athletes with recorded timing
    - Orders fastest to slowest
    """
    ...
    entries_qs = (
        ...
        .filter(
            ...
            participation__total_lap_time__isnull=False,
        )
    )

    # Only show results for the race clicked
    if race_id:
        entries_qs = entries_qs.filter(
            race_id=race_id
        )

    # Fastest athlete first
    entries_qs = entries_qs.order_by(
        "participation__total_lap_time"
    )
    ...
    for e in entries:
        ...
        # Fallback to Participation timing
        try:

            if (
                not e.total_lap_time
                and getattr(
                    e,
                    "participation",
                    None
