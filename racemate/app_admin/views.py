...
#     # Replace with this:
#     schools = Registration.objects.values_list('school_name', flat=True).distinct().order_by('school_name')
...
from .forms import RegistrationForm
from app_admin.models import DimState, DimDistrict, DimEventCategory
from app_races.models import RaceRegistration
...
def build_filtered_qs(request):
    """
    Returns filtered queryset of Registration based on GET params.
    Events are now linked via RaceRegistration → Event, not directly on Registration.
    """
    qs = Registration.objects.all()
    ...
    # Event filter — now via RaceRegistration → event
    event_ids = request.GET.getlist('event')
    if event_ids:
        qs = qs.filter(race_entries__event_id__in=event_ids)

    # School filter (replaces profession)
...
def api_analysis_summary(request):
    """
    Returns analysis summary JSON.
    Events are traversed via participation (RaceRegistration) → event.
    """
    qs = build_filtered_qs(request)
    ...
    # By State
    by_state_qs = (
        qs.values('state__id', 'state__name')
        .annotate(count=Count('id'))
        .order_by('-count')
    )
    by_state = [
        {'id': r['state__id'], 'name': r['state__name'] or 'Unknown', 'count': r['count']}
        for r in by_state_qs
    ]

    # By Event — via RaceRegistration
    by_event_qs = (
        qs.values('race_entries__event__id', 'race_entries__event__title')
        .annotate(count=Count('id'))
        .order_by('-count')
    )
    by_event = [
        {
            'id': r['race_entries__event__id'],
            'name': r['race_entries__event__title'] or 'Unknown',
            'count': r['count']
        }
        for r in by_event_qs
    ]

    # By Gender
    by_gender_qs = qs.values('gender').annotate(count=Count('id')).order_by('gender')
    by_gender = [
        {'gender': r['gender'] or 'Unspecified', 'count': r['count']}
        for r in by_gender_qs
    ]

    # By Category (age group)
    by_category_qs = qs.values('category').annotate(count=Count('id')).order_by('category')
    by_category = [
        {'age_group': r['category'] or 'Unspecified', 'count': r['count']}
        for r in by_category_qs
    ]

    # Detailed Breakdown (category + event + gender) — via RaceRegistration
    breakdown_qs = (
        qs.values('category', 'race_entries__event__title', 'gender')
        .annotate(count=Count('id'))
        .order_by('category', 'race_entries__event__title', 'gender')
    )
    breakdown = [
        {
            'age_group': r['category'] or 'Unspecified',
            'event': r['race_entries__event__title'] or 'Unspecified',
            'gender': r['gender'] or 'Unspecified',
            'count': r['count'],
        }
        for r in breakdown_qs
...
def analysis_export_csv(request):
    ...
    Events fetched via RaceRegistration.
    """
    qs = (
        build_filtered_qs(request)
        .prefetch_related('race_entries__event')
        .select_related('state', 'district_fk')
        .order_by('-created_at')
    )
    ...
    writer.writerow([
        ...
        'school', 'state', 'district', 'events', 'category', 'created_at'
    ])

    for r in qs.iterator():
        events = "; ".join(
            rr.event.title for rr in r.race_entries.all() if rr.event
        )
        writer.writerow([
            r.id,
            r.name,
...
def analysis_dashboard(request):
    """
    Render analysis dashboard with filter options.
    """
    states = DimState.objects.all().order_by('name')
    events = DimEventCategory.objects.all().order_by('name')
    schools = Registration.objects.values_list('school_name', flat=True).distinct().order_by('school_name')
    ...
    genders = [
        {'value': g[0], 'label': g[1]}
        for g in Registration.GENDER_CHOICES
    ]
